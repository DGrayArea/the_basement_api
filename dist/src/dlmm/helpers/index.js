...
const getOrCreateATAInstruction = async (connection, tokenMint, owner, programId, payer = owner, allowOwnerOffCurve = true) => {
    programId = programId ?? spl_token_1.TOKEN_PROGRAM_ID;
    const toAccount = (0, spl_token_1.getAssociatedTokenAddressSync)(tokenMint, owner, allowOwnerOffCurve, programId, spl_token_1.ASSOCIATED_TOKEN_PROGRAM_ID);
    try {
        await (0, spl_token_1.getAccount)(connection, toAccount, connection.commitment, programId);
        ...
        if (e instanceof spl_token_1.TokenAccountNotFoundError ||
            ...
            const ix = (0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(payer, toAccount, owner, tokenMint, programId, spl_token_1.ASSOCIATED_TOKEN_PROGRAM_ID);
...
const getEstimatedComputeUnitUsageWithBuffer = async (connection, instructions, feePayer, buffer) => {
    ...
    const estimatedComputeUnitUsage = await (0, computeUnit_1.getSimulationComputeUnits)(connection, instructions, feePayer, []);
